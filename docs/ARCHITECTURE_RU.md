...
- Состояние пользователей хранится в `TRNS_STATE_DIR` (`state/users/<telegram_id>.json` по умолчанию); `config.json` `allowed_user_ids` остаётся только для старых/предварительно разрешённых пользователей
...
- `state/`: состояние пользователей и дневной лимит LM (`state/global.json`)
